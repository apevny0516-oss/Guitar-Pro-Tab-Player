...
class TabPlayer {
    constructor() {
        ...
        // Seamless loop tracking
        this.loopStartTick = 0;
        this.loopEndTick = 0;
        this.isSeamlessLooping = false;
    ...
    initAlphaTab() {
        ...
        // Continuous scroll tracking during playback
    ...
    startScrollTracking() {
        const trackCursor = () => {
            ...
            this.scrollAnimationId = requestAnimationFrame(trackCursor);
        };
        trackCursor();
    }
    
    keepCursorInView() {
        const cursor = this.elements.container.querySelector('.at-cursor-beat');
        if (!cursor) return;
        ...
        // Target position: keep cursor about 1/3 from the top of the visible area
        const targetY = containerRect.top + (containerRect.height * 0.33);
        const currentY = cursorRect.top;
        const diff = currentY - targetY;
        
        // Only scroll if cursor is significantly off from target position
        if (Math.abs(diff) > 20) {
            // Smooth scrolling - move towards target gradually
    ...
    toggleLoop() {
        ...
        // Only use alphaTab's built-in looping when not using seamless looping
        if (!this.isSeamlessLooping) {
    ...
    onPlaybackRangeChanged(args) {
        ...
        if (args.playbackRange && args.playbackRange.startTick >= 0 && args.playbackRange.endTick > args.playbackRange.startTick) {
            ...
            // Store loop range for seamless looping
            this.loopStartTick = args.playbackRange.startTick;
            this.loopEndTick = args.playbackRange.endTick;
            this.isSeamlessLooping = true;
            
            // Disable alphaTab's built-in looping - we'll handle it manually for seamless loops
            this.api.isLooping = false;
            
            // Keep UI showing loop is active
            if (!this.isLooping) {
                this.isLooping = true;
    ...
    onPositionChanged(args) {
        ...
        if (this.isSeamlessLooping && this.isPlaying && this.isLooping && !this.loopJumpPending) {
            const currentTick = args.currentTick;
            
            // Calculate the full duration of the loop in ticks
            // Add a small buffer to let the last note ring out naturally
            // The buffer is approximately 1/16th of a beat (960 ticks per quarter note = 60 ticks)
            const noteRingBuffer = 60;
            
            // Jump back when we've passed the end tick plus the ring buffer
            if (currentTick >= this.loopEndTick + noteRingBuffer) {
                // Prevent multiple jumps while seeking
                this.loopJumpPending = true;
                
                // Jump back to loop start
                this.api.tickPosition = this.loopStartTick;
